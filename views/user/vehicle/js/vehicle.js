function brandList() {
  ...
  $.ajax({
    type: 'POST',
    url: 'models/vehicle/brandList.php',
    dataType: 'json',
    data: {
      id_page: idp
    ...
    success: function (data) {
      ...
      for (i = 0; i < data.BrandName.length; i++) {
        $('#marque').append('<option id="listeMarque"value="">'+BrandName[i]+'</option>');
...
function modelList() {
  ...
  $.ajax({
    type: 'POST',
    url: 'models/vehicle/modelList.php',
    dataType: 'json',
    data: {
      id_page: idp
    ...
    success: function (data) {
      ...
      for (i = 0; i < data.ModelName.length; i++) {
        $('#modele').append('<h2>'+ModelName[i]+'</h2>');
